...
import {
  Wallet, Container, BarChart3, Database, Globe, ShieldCheck,
...
const steps: JourneyStep[] = [
  {
    id: 'challenge',
    icon: Wallet,
    title: 'Black Friday exige infraestrutura robusta',
    narrative: 'O Bitz precisava migrar para a nuvem AWS a tempo da Black Friday, garantindo continuidade das operações financeiras com menor custo e máxima segurança.',
    before: 'Infraestrutura incapaz de suportar o volume transacional da Black Friday',
    after: 'Ambiente AWS pronto para operar com alta disponibilidade no pico de demanda',
    color: '#d97706',
  },
  {
    id: 'containers',
    icon: Container,
    title: 'Containers com Auto Scaling no ECS',
    narrative: 'AWS ECS com Auto Scaling dimensiona containers automaticamente com base em CPU e tráfego. Se um container falha, outro assume instantaneamente.',
    before: 'Containers sem escalabilidade automática — falhas causavam indisponibilidade',
    after: 'Auto Scaling garante substituição automática e dimensionamento por demanda',
    highlight: 'Alta disponibilidade com ECS',
    color: '#2563eb',
  },
  {
    id: 'loadbalancer',
    icon: BarChart3,
    title: 'Distribuição inteligente de tráfego',
    narrative: 'Application Load Balancer integrado ao ECS distribui tráfego entre containers. Em caso de falha, o tráfego é redirecionado automaticamente.',
    before: 'Tráfego concentrado sem balanceamento — risco de sobrecarga',
    after: 'Balanceamento automático com redirecionamento em caso de falha',
    color: '#7c3aed',
  },
  {
    id: 'database',
    icon: Database,
    title: 'Banco de dados resiliente com Multi-AZ',
    narrative: 'RDS configurado com Multi-AZ garante que, em caso de interrupção em uma zona, as requisições são redirecionadas automaticamente para uma zona saudável.',
    before: 'Banco de dados em zona única — vulnerável a falhas regionais',
    after: 'Multi-AZ com failover automático para continuidade total do serviço',
    color: '#dc2626',
  },
  {
    id: 'frontend',
    icon: Globe,
    title: 'Frontend escalável com S3 + CloudFront',
    narrative: 'O frontend é armazenado no S3 e entregue pelo CloudFront, ambos configurados para serem escaláveis e altamente disponíveis por padrão.',
    before: 'Frontend hospedado em infraestrutura limitada e sem CDN',
    after: 'Entrega global com S3 + CloudFront — escalável e rápido',
    color: '#0d9488',
  },
  {
    id: 'results',
    icon: ShieldCheck,
    title: 'DevOps que garante o sucesso',
    narrative: 'A abordagem DevOps da dataRain possibilitou uma implantação em 60 dias com escalabilidade, disponibilidade e segurança. O Bitz operou sem falhas na Black Friday.',
    before: 'Risco de indisponibilidade em um dos maiores eventos de vendas do ano',
    after: 'Operação impecável na Black Friday com zero downtime',
    highlight: 'Zero downtime na Black Friday',
    color: '#059669',
  },
];

export const BitzDevOpsDemo: React.FC = () => {
      ...
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-slate-700">Jornada do Bitz — DevOps para a Black Friday</span>
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">DevOps da dataRain garantiu operação impecável do Bitz na Black Friday.</p><p className="text-xs text-slate-500 mt-1">Escalabilidade, segurança e alta disponibilidade em 60 dias.</p></motion.div>}
        </div>
      </div>
    </div>
  );
