...
import { GitBranch, Code2, Users, CheckCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
const steps: JourneyStep[] = [
  { id: 'client', icon: GitBranch, title: 'Migração GitLab', narrative: 'Aarin TechFin precisava migrar toda infraestrutura GitLab entre instâncias: projetos, grupos, pipelines CI/CD e usuários.', before: 'GitLab em instância antiga sem migração planejada', after: 'Migração completa automatizada via API GitLab', color: '#e65100' },
  { id: 'scripts', icon: Code2, title: 'Scripts de migração via API', narrative: 'Scripts cURL via API GitLab para export/dump de projetos entre instâncias. Migração de imagens de dependência de pipeline.', before: 'Migração manual projeto a projeto', after: 'Scripts automatizados via API para export/import', color: '#7c3aed' },
  { id: 'users', icon: Users, title: 'Migração de usuários', narrative: 'Script automatizado de invite de usuários na nova instância. Assessment e reorganização da estrutura de grupos.', before: 'Usuários e permissões sem migração', after: 'Script de invite + reorganização de grupos', color: '#0891b2' },
  { id: 'results', icon: CheckCircle, title: 'Pipeline validada', narrative: 'Pipelines CI/CD ajustadas e validadas na nova instância. Estrutura de grupos reorganizada.', before: 'Pipelines CI/CD desconfiguradas', after: 'Pipelines ajustadas e validadas pós-migração', color: '#059669' },
];

export const AarinDemo: React.FC = () => {
    ...
    <div className="w-full max-w-4xl mx-auto my-8">
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">Migração GitLab completa com projetos, pipelines e usuários automatizados.</p></motion.div>}
        </div>
      </div>
    </div>
  );
