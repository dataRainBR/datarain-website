...
import {
  Headphones, Bot, MessageSquare, Shield, Target, Rocket,
...
const steps: JourneyStep[] = [
  { id: 'challenge', icon: Headphones, title: '80 mil atendimentos por mês', narrative: 'O SAC do Ailos processava 80 mil atendimentos mensais entre ligações e WhatsApp, com alto TMA, base de conhecimento fragmentada e falta de padronização nas respostas.', before: 'Alto TMA, TME elevado e taxa de abandono significativa', after: 'Copiloto de IA sugerindo respostas em tempo real aos atendentes', highlight: '80 mil atendimentos/mês', color: '#2563eb' },
  { id: 'copilot', icon: Bot, title: 'Copiloto com Amazon Bedrock', narrative: 'Agente virtual inteligente integrado ao Microsoft Teams, utilizando Amazon Bedrock como LLM com flexibilidade para troca de modelo (Anthropic, Mistral, Meta).', before: 'Atendentes consultando múltiplas fontes manualmente', after: 'IA sugere respostas contextualizadas em tempo real via Teams', color: '#7c3aed' },
  { id: 'knowledge', icon: MessageSquare, title: 'Base de conhecimento inteligente', narrative: 'Repositório versionado e segmentado por cooperativa e produto, com cache inteligente, feedback thumbs up/down e fallback para especialista humano.', before: 'Base fragmentada por cooperativa, sem segmentação', after: 'Repositório unificado, versionado e segmentado com feedback loop', color: '#d97706' },
  { id: 'security', icon: Shield, title: 'Segurança e LGPD', narrative: 'Conformidade com LGPD, criptografia TLS 1.2+ e AES-256, Zero Trust Architecture, mascaramento automático de CPF e telefones, content filtering e circuit breaker.', before: 'Dados sensíveis sem mascaramento automático', after: 'LGPD, Zero Trust, mascaramento automático e content filtering', color: '#dc2626' },
  { id: 'goals', icon: Target, title: 'Metas ambiciosas', narrative: 'Acurácia ≥90%, taxa de alucinação ≤10%, tempo de resposta <10s, disponibilidade 99,5%, suporte a 100 consultas simultâneas e satisfação ≥90% dos usuários piloto.', before: 'Sem métricas de qualidade para o atendimento assistido por IA', after: 'Metas claras: ≥90% acurácia, <10s resposta, 99,5% disponibilidade', highlight: '≥90% acurácia | ≤10% alucinação', color: '#0d9488' },
  { id: 'results', icon: Rocket, title: 'SAC transformado por IA', narrative: 'Framework de IA para escalonamento futuro, comitê de IA, curadoria contínua, logs auditáveis e CI/CD com testes automatizados para evolução segura.', before: 'Atendimento manual sem inteligência artificial', after: 'SAC potencializado por IA Generativa com governança completa', color: '#059669' },
];

export const AilosDemo: React.FC = () => {
    ...
    <div className="w-full max-w-4xl mx-auto my-8">
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">IA Generativa transformou o SAC do Ailos com copiloto inteligente.</p><p className="text-xs text-slate-500 mt-1">80 mil atendimentos/mês potencializados por Amazon Bedrock.</p></motion.div>}
        </div>
      </div>
    </div>
  );
