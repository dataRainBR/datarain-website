...
import { BarChart3, DollarSign, Cloud, Server, Shield, Users, ChevronLeft, ChevronRight } from 'lucide-react';
...
const steps: JourneyStep[] = [
  {
    id: 'challenge',
    icon: BarChart3,
    title: '40 anos de dados que precisam escalar',
    narrative: 'A Fundação SEADE é referência nacional em estatísticas socioeconômicas. Com o volume de dados crescendo, a infraestrutura on-premises não acompanhava mais.',
    before: 'Infraestrutura on-premises sem capacidade de escalar com o volume crescente de dados',
    after: 'Ambiente em nuvem AWS preparado para crescer com a demanda',
    color: '#2563eb',
  },
  {
    id: 'cost',
    icon: DollarSign,
    title: 'De US$ 5M para US$ 300K',
    narrative: 'Modernizar o parque tecnológico custaria US$ 5 milhões. Hospedagem tradicional, US$ 1,25M. A migração para AWS com a dataRain custou apenas US$ 300K — 1/60 do CAPEX original.',
    before: 'Custo de modernização proibitivo: US$ 5 milhões em CAPEX',
    after: 'Migração para AWS por US$ 300K — economia de 94% sobre o CAPEX',
    highlight: '94% de economia',
    color: '#059669',
  },
  {
    id: 'migration',
    icon: Cloud,
    title: 'Migração IaaS para AWS',
    narrative: 'Licitação pública vencida pela dataRain com plano de migração IaaS. EC2, EBS, S3, Route 53 e VPC formam a base da nova infraestrutura.',
    before: 'Processo licitatório complexo com múltiplas opções no mercado',
    after: 'Solução AWS vencedora com melhor custo-benefício e capacidade técnica',
    color: '#7c3aed',
  },
  {
    id: 'infra',
    icon: Server,
    title: 'Infraestrutura moderna e gerenciada',
    narrative: 'Setup completo com consultoria, gerenciamento de ambiente e operação DevOps. A dataRain se tornou trusted advisor do SEADE para assuntos de nuvem.',
    before: 'Equipe interna sem experiência em nuvem e sem suporte especializado',
    after: 'Operação gerenciada com consultoria contínua e suporte DevOps',
    color: '#d97706',
  },
  {
    id: 'security',
    icon: Shield,
    title: 'Dados públicos com segurança',
    narrative: 'Armazenamento seguro com S3, rede isolada com VPC e DNS gerenciado com Route 53. Dados públicos acessíveis com a segurança que o governo exige.',
    before: 'Dados sensíveis em infraestrutura vulnerável e sem isolamento adequado',
    after: 'Ambiente seguro com VPC, S3 e políticas de acesso adequadas',
    color: '#dc2626',
  },
  {
    id: 'trust',
    ...
    narrative: 'O profissionalismo e transparência da dataRain fizeram do SEADE um cliente de longo prazo. A AWS e a dataRain são agora parceiros estratégicos da fundação.',
    before: 'Dependência de infraestrutura cara e sem parceiro de confiança para TI',
    after: 'Trusted advisor para nuvem com suporte contínuo e transparente',
    color: '#0d9488',
  },
];

export const SeadeDemo: React.FC = () => {
      ...
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-slate-700">Jornada do SEADE — Modernização Tecnológica na Nuvem</span>
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">A dataRain levou o SEADE do on-premises à nuvem AWS com economia de 95%.</p><p className="text-xs text-slate-500 mt-1">Dados públicos acessíveis com infraestrutura moderna e econômica.</p></motion.div>}
        </div>
      </div>
    </div>
  );
