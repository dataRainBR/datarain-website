...
import {
  ShieldAlert, Search, BookOpen, ShieldCheck, BarChart3, Rocket,
...
const steps: JourneyStep[] = [
  { id: 'challenge', icon: ShieldAlert, title: 'Gaps críticos de segurança', narrative: 'Apesar de operar na AWS, o Grupo BMG apresentava lacunas significativas: certificados expirando, snapshots sem criptografia, S3 sem versionamento e score de maturidade de 1.6/10.', before: 'Score de maturidade de segurança de 1.6 em 10', after: 'Assessment completo com roadmap de remediação priorizado', highlight: '19+ findings críticos identificados', color: '#dc2626' },
  { id: 'scan', icon: Search, title: 'Varredura com Prowler', narrative: 'Varredura automatizada de conformidade em toda a organização AWS com Prowler, gerando Security Report detalhado com findings categorizados por domínio.', before: 'Sem visibilidade sobre o estado real da segurança', after: 'Relatório completo com findings categorizados e priorizados', color: '#2563eb' },
  { id: 'workshops', icon: BookOpen, title: 'Workshops dedicados', narrative: 'Três workshops práticos com as equipes BMG: Data Protection (criptografia, KMS, S3), Detective Controls (CloudTrail, Config) e Incident Response (automação, playbooks).', before: 'Equipes sem conhecimento aprofundado em segurança AWS', after: 'Equipes capacitadas em Data Protection, Detective Controls e Incident Response', color: '#7c3aed' },
  { id: 'guardrails', icon: ShieldCheck, title: 'Guardrails e SCPs', narrative: 'Implementação de Service Control Policies para prevenção automatizada de configurações inseguras, com estratégia de notificações e backlog priorizado.', before: 'Sem controles preventivos automatizados', after: 'SCPs implementadas com guardrails e notificações automáticas', color: '#d97706' },
  { id: 'heatmap', icon: BarChart3, title: 'Heatmap de maturidade', narrative: 'Assessment Tool com summary executivo e heatmap por domínio: General Security, Data Protection, Infrastructure Security, IAM, Detective Controls, Incident Response.', before: 'Sem métricas objetivas de maturidade de segurança', after: 'Heatmap visual por domínio com baseline para evolução', color: '#0d9488' },
  { id: 'results', icon: Rocket, title: 'Postura de segurança elevada', narrative: 'Backlog de segurança criado com owners e prazos, guardrails implementados e equipe BMG capacitada para manter e evoluir a segurança continuamente.', before: 'Segurança reativa sem processo estruturado', after: 'Segurança proativa com backlog, owners e guardrails automatizados', color: '#059669' },
];

export const BMGDemo: React.FC = () => {
    ...
    <div className="w-full max-w-4xl mx-auto my-8">
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">Assessment completo elevou a postura de segurança do Grupo BMG na AWS.</p><p className="text-xs text-slate-500 mt-1">19+ findings remediados com guardrails e capacitação.</p></motion.div>}
        </div>
      </div>
    </div>
  );
