...
import {
  CreditCard, Code2, Shield, Activity, GitBranch, Rocket,
...
const steps: JourneyStep[] = [
  {
    id: 'mission',
    icon: CreditCard,
    title: 'Inclusão financeira via PIX',
    narrative: 'A BluePay é uma fintech especializada em PIX, focada em bancarizar profissionais autônomos e prestadores de serviços com acesso simplificado a serviços financeiros.',
    before: 'Profissionais autônomos sem acesso fácil a serviços bancários essenciais',
    after: 'Plataforma de pagamentos acessível, escalável e focada em inclusão financeira',
    color: '#2563eb',
  },
  {
    id: 'serverless',
    icon: Code2,
    title: 'Arquitetura 100% serverless',
    narrative: 'A BluePay construiu todo o ambiente com Lambda, API Gateway e DynamoDB/RDS. Implantação em São Paulo com três zonas de disponibilidade.',
    before: 'Necessidade de infraestrutura que escalasse sem custos fixos elevados',
    after: 'Ambiente serverless com escalabilidade automática e custos sob demanda',
    highlight: 'Zero servidores para gerenciar',
    color: '#7c3aed',
  },
  {
    id: 'security',
    ...
    narrative: 'Security Groups restritivas, VPC Flow Logs, criptografia TLS/SSL via ACM, CloudFront com HTTPS e chaves no KMS. Tudo seguindo o princípio do menor privilégio.',
    before: 'Controles de segurança básicos insuficientes para o setor financeiro',
    after: 'Múltiplas camadas de segurança com criptografia ponta a ponta',
    color: '#dc2626',
  },
  {
    id: 'monitoring',
    icon: Activity,
    title: 'Monitoramento completo com CloudWatch',
    narrative: 'Painel personalizado cobrindo infraestrutura, aplicação e erros. Visualização fácil das informações essenciais para operação contínua.',
    before: 'Visibilidade limitada sobre a saúde da aplicação e infraestrutura',
  ...
  },
  {
    id: 'cicd',
    icon: GitBranch,
    title: 'CI/CD com Infrastructure as Code',
    narrative: 'Serverless Framework com CloudFormation e pipeline no GitHub. Alterações em produção passam por um fluxo automatizado e auditável.',
    before: 'Deploys manuais, lentos e propensos a erros humanos',
    after: 'Pipeline automatizado: do commit ao deploy em produção com segurança',
    highlight: 'Deploy automatizado e auditável',
    color: '#0d9488',
  },
  {
    id: 'results',
    icon: Rocket,
    title: 'Agilidade que transforma o negócio',
    narrative: 'Com DevOps e serverless, a BluePay entrega funcionalidades mais rápido, escala sob demanda e opera com a confiabilidade que o mercado financeiro exige.',
    before: 'Ciclos longos de desenvolvimento e entrega de novas funcionalidades',
    after: 'Entregas frequentes, escalabilidade automática e alta confiabilidade',
    color: '#059669',
  },
];

export const BluePayDemo: React.FC = () => {
      ...
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-slate-700">Jornada da BluePay — DevOps Serverless para Fintech</span>
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">DevOps serverless transformou a BluePay em uma fintech ágil e segura.</p><p className="text-xs text-slate-500 mt-1">Inclusão financeira com tecnologia de ponta na AWS.</p></motion.div>}
        </div>
      </div>
    </div>
  );
