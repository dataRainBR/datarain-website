...
import { Zap, Code2, Shield, Activity, GitBranch, Rocket, ChevronLeft, ChevronRight } from 'lucide-react';
...
const steps: JourneyStep[] = [
  {
    id: 'challenge',
    icon: Zap,
    title: 'Energia que precisa de escala',
    narrative: 'A Intelbras, com mais de 40 anos de experiência, precisava de infraestrutura capaz de acompanhar o crescimento do negócio de energia com escalabilidade, segurança e confiabilidade.',
    before: 'Infraestrutura que não acompanhava o ritmo de crescimento do negócio',
    after: 'Ambiente AWS escalável e preparado para o crescimento contínuo',
    color: '#d97706',
  },
  {
    id: 'serverless',
    icon: Code2,
    title: 'Serverless com Lambda e Fargate',
    narrative: 'Abordagem serverless com Lambda e Fargate, construída com Serverless Framework via CloudFormation. Três zonas de disponibilidade em São Paulo.',
    before: 'Infraestrutura tradicional com provisionamento manual e lento',
    after: 'Serverless com deploy automatizado e três zonas de disponibilidade',
    highlight: '3 AZs em São Paulo',
    color: '#2563eb',
  },
  {
    id: 'access',
    ...
    narrative: 'AWS Identity Center com SSO entre contas, credenciais temporárias e IAM exclusivo para pipelines. Princípio do menor privilégio em todo o ambiente.',
    before: 'Gerenciamento de acesso manual e sem padronização entre contas',
    after: 'SSO centralizado com credenciais temporárias e menor privilégio',
    color: '#dc2626',
  },
  {
    id: 'monitoring',
    icon: Activity,
    title: 'Health check personalizado',
    narrative: 'Aplicação customizada monitora conexões de banco, APIs internas, filas e dead letter queues. CloudWatch centraliza logs com painel personalizado.',
    before: 'Sem monitoramento proativo — problemas descobertos apenas quando impactavam usuários',
    after: 'Health checks automáticos com monitoramento proativo de toda a stack',
    color: '#7c3aed',
  },
  {
    id: 'testing',
    icon: GitBranch,
    title: 'Testes automatizados no pipeline',
    narrative: 'Pipeline com testes unitários em Node.js usando Vitest. Terraform para mudanças de infraestrutura com fluxo estruturado. Snapshots diários para DR.',
    before: 'Sem testes automatizados — bugs chegavam à produção com frequência',
    after: 'Testes automatizados no pipeline com Vitest e IaC com Terraform',
    highlight: 'Qualidade garantida no pipeline',
    color: '#0d9488',
  },
  {
    id: 'results',
    icon: Rocket,
    title: 'Eficiência operacional transformada',
    narrative: 'Gestão eficiente, deploys automatizados, alta disponibilidade em 3 AZs e monitoramento proativo. A Intelbras agora inova com a velocidade que o mercado exige.',
    before: 'Ciclos longos de desenvolvimento e operação reativa',
    after: 'Entregas ágeis, infraestrutura resiliente e operação proativa',
    color: '#059669',
  },
];

export const IntelbrasDemo: React.FC = () => {
      ...
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-slate-700">Jornada da Intelbras — DevOps para Energia e Inovação</span>
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">DevOps da dataRain deu à Intelbras a infraestrutura para inovar com velocidade.</p><p className="text-xs text-slate-500 mt-1">Mais de 40 anos de experiência, agora com a agilidade da nuvem.</p></motion.div>}
        </div>
      </div>
    </div>
  );
