...
import { Dna, Cpu, Lock, ChevronLeft, ChevronRight } from 'lucide-react';
...
const steps: JourneyStep[] = [
  { id: 'client', icon: Dna, title: 'Biotecnologia e genômica', narrative: 'A Symbiomics é especializada em análise genômica, atuando na fronteira entre bioinformática e ciência de dados com pipelines que processam grandes volumes de dados genômicos.', before: 'Pipelines de genômica limitados por infraestrutura local', after: 'Pipeline de análise genômica na AWS com EC2 otimizado', color: '#7c3aed' },
  { id: 'compute', icon: Cpu, title: 'EC2 para bioinformática', narrative: 'Instâncias EC2 dimensionadas para workloads de genômica com análise de pricing para otimização de custos entre diferentes tipos de instância.', before: 'Sem capacidade computacional adequada para genômica', after: 'EC2 dimensionado e precificado para bioinformática', highlight: 'Instâncias otimizadas para genômica', color: '#0891b2' },
  { id: 'access', icon: Lock, title: 'Acesso seguro via OpenVPN', narrative: 'OpenVPN configurado para acesso seguro dos pesquisadores ao ambiente de processamento, com controle de acesso por chaves dedicadas e isolamento de rede.', before: 'Acesso remoto sem segurança adequada', after: 'OpenVPN com chaves dedicadas e isolamento de rede', color: '#059669' },
];

export const SymbiomicsDemo: React.FC = () => {
    ...
    <div className="w-full max-w-4xl mx-auto my-8">
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">Symbiomics executa pipelines de genômica na AWS com acesso seguro.</p></motion.div>}
        </div>
      </div>
    </div>
  );
