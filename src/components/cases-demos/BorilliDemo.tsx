...
import { Server, Lock, Shield, Activity, ChevronLeft, ChevronRight } from 'lucide-react';
...
const steps: JourneyStep[] = [
  { id: 'client', icon: Server, title: 'Infraestrutura para pneus', narrative: 'Borilli Pneus precisava de ambiente AWS completo para servidores de aplicação Wildfly/Junsoft.', before: 'Sem infraestrutura cloud para aplicação', after: '4 EC2 provisionados na AWS', color: '#37474f' },
  { id: 'vpn', icon: Lock, title: 'VPN + Sophos XG 105', narrative: 'VPN Client-to-Site com OpenVPN para acesso remoto seguro. Firewall Sophos XG 105 para proteção de rede.', before: 'Sem VPN e sem firewall', after: 'OpenVPN + Sophos XG 105 configurados', color: '#7c3aed' },
  { id: 'security', icon: Shield, title: 'IAM + MFA + Secrets Manager', narrative: 'IAM com MFA obrigatório e Secrets Manager para gestão de credenciais. Security Groups restritivos.', before: 'Sem MFA e credenciais em texto plano', after: 'IAM + MFA + Secrets Manager configurados', color: '#dc2626' },
  { id: 'monitoring', icon: Activity, title: 'Monitoramento + backup', narrative: 'Site24x7 + SSM para monitoramento. Backup automatizado com snapshots + S3. Lambda para automação.', before: 'Sem monitoramento e backup manual', after: 'Site24x7 + SSM + backup automatizado S3', color: '#059669' },
];

export const BorilliDemo: React.FC = () => {
    ...
    <div className="w-full max-w-4xl mx-auto my-8">
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">Ambiente AWS provisionado com 4 EC2, VPN e monitoramento ativo.</p></motion.div>}
        </div>
      </div>
    </div>
  );
