...
import {
  Factory, Database, Cpu, BarChart3, Zap, Users,
...
const steps: JourneyStep[] = [
  {
    id: 'legacy',
    icon: Factory,
    title: 'Dados industriais presos em sistemas legados',
    narrative: 'A PQ gerenciava dados industriais com um historiador tradicional, Kepware e TimescaleDB. O modelo de licenciamento era rígido e a administração exigia especialistas dedicados.',
    before: 'Sistema legado complexo, licenciamento inflexível e lógica de negócios concentrada no historiador',
    after: 'Arquitetura moderna, serverless e com custos sob demanda na AWS',
    color: '#d97706',
  },
  {
    id: 'edge',
    icon: Cpu,
    title: 'Processamento inteligente na borda',
    narrative: 'Sensores e PLCs enviam dados ao gateway IoT com Kepware. O Amazon Greengrass executa scripts Python para filtragem e pré-processamento, enviando apenas o essencial para a nuvem.',
    before: 'Todos os dados brutos trafegavam para a nuvem, gerando custo e sobrecarga desnecessários',
    after: 'Processamento na borda com Greengrass filtra dados irrelevantes antes do envio',
    highlight: 'Redução significativa de tráfego',
    color: '#2563eb',
  },
  {
    id: 'streaming',
    icon: Zap,
    title: 'Streaming em tempo real com Kinesis',
    narrative: 'O Amazon Kinesis gerencia o volume de dados em tempo real. Funções Lambda automatizam a ingestão no TimescaleDB, banco otimizado para séries temporais.',
    before: 'Dados demoravam a chegar e dependiam de processos manuais de ingestão',
    after: 'Fluxo contínuo e automatizado do chão de fábrica à nuvem, quase em tempo real',
    color: '#7c3aed',
  },
  {
    id: 'data',
    icon: Database,
    title: 'Dados mestres centralizados e acessíveis',
    narrative: 'Dados mestres ficam em SQL Server, acessíveis via Excel. Engenheiros consultam e atualizam informações sem depender de sistemas legados ou especialistas externos.',
    before: 'Dependência de especialistas para qualquer consulta ou atualização de dados',
    after: 'Times internos com autonomia total para acessar e gerenciar dados mestres',
    color: '#0d9488',
  },
  {
    id: 'dashboards',
    icon: BarChart3,
    title: 'Dashboards acionáveis com Seeq',
    narrative: 'A plataforma Seeq transforma dados em dashboards e análises visuais. Manutenção preditiva e acompanhamento de performance agora são acessíveis e confiáveis.',
    before: 'Informações chegavam com atraso e sem confiabilidade para tomada de decisão',
    after: 'Dashboards em tempo real, confiáveis e prontos para análise preditiva',
    highlight: 'Menos latência, mais decisões em tempo real',
    color: '#dc2626',
  },
  {
    id: 'partnership',
    ...
    narrative: '"A dataRain tem sido nossa parceira estratégica há alguns anos e esteve ao nosso lado em todos os projetos de sucesso da jornada de transformação de dados da PQ." — Adriana Adrioli, IT Manager',
    before: 'Infraestrutura rígida que não acompanhava o crescimento do negócio',
    after: 'Arquitetura elástica e serverless que escala com a operação',
    highlight: 'Autonomia e escalabilidade',
    color: '#059669',
  },
];

export const PQDemo: React.FC = () => {
      ...
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-slate-700">Jornada da PQ — Modernização de Dados Industriais</span>
          ...
          {isLast && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200">
              <p className="text-sm text-emerald-700 font-medium">Da fábrica à nuvem: a PQ agora opera com dados em tempo real e total autonomia.</p>
              <p className="text-xs text-slate-500 mt-1">Parceria estratégica dataRain + AWS transformando a indústria química.</p>
            </motion.div>
          )}
        </div>
      </div>
