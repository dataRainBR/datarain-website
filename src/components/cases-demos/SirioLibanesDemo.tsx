...
import {
  FileWarning, GitCompareArrows, Workflow, ShieldCheck, Stethoscope, Rocket,
...
const steps: JourneyStep[] = [
  {
    id: 'silos',
    icon: FileWarning,
    title: 'Dados médicos isolados',
    narrative: 'O Hospital Sírio-Libanês opera com um protocolo interno próprio (HSL) para registrar dados de pacientes. Mas esse formato não conversa com outros sistemas de saúde — cada hospital, laboratório e clínica fala uma "língua" diferente.',
    before: 'Dados presos em formato proprietário, sem possibilidade de troca com outros sistemas',
    after: 'Dados prontos para serem convertidos e compartilhados no padrão global FHIR',
    color: '#d97706',
  },
  {
    id: 'mapping',
    icon: GitCompareArrows,
    title: 'Mapeamento e tradução',
    narrative: 'A dataRain criou uma interface inteligente que mapeia cada campo do protocolo HSL e o converte para o padrão FHIR (Fast Healthcare Interoperability Resources) — o idioma universal da saúde digital.',
    before: 'Conversão manual, sujeita a erros, lenta e dependente de especialistas',
    after: 'Interface automatizada com canal bidirecional: HSL ↔ FHIR em tempo real',
    highlight: 'Comunicação bidirecional HSL ↔ FHIR',
    color: '#2563eb',
  },
  {
    id: 'processing',
    icon: Workflow,
    title: 'Processamento serverless',
    narrative: 'Cada conversão de dados é processada por funções serverless que escalam automaticamente. Não importa se são 10 ou 10.000 registros — o sistema responde sob demanda, sem servidores ociosos.',
    before: 'Servidores dedicados com capacidade fixa — gargalo em picos, desperdício em vales',
    after: 'Processamento elástico que escala com a demanda, pagando apenas pelo uso',
    highlight: 'Escala automática · Custo sob demanda',
    color: '#7c3aed',
  },
  {
    id: 'storage',
    ...
    narrative: 'Os dados FHIR gerados são armazenados, consultados e validados em um repositório especializado em saúde, garantindo conformidade com regulamentações e segurança dos dados dos pacientes.',
    before: 'Armazenamento genérico sem validação de conformidade com padrões de saúde',
    after: 'Repositório especializado com validação automática e conformidade regulatória',
    color: '#0d9488',
  },
  {
    id: 'care',
    icon: Stethoscope,
    title: 'Melhor cuidado ao paciente',
    narrative: 'Com dados integrados e interoperáveis, médicos de diferentes instituições podem acessar o histórico completo do paciente. Diagnósticos mais precisos, tratamentos mais eficazes, menos exames repetidos.',
    before: 'Histórico fragmentado — cada hospital vê apenas seus próprios dados',
    after: 'Visão unificada do paciente, compartilhável entre instituições de saúde',
    highlight: 'Histórico completo · Diagnóstico mais preciso',
    color: '#db2777',
  },
  {
    id: 'future',
    ...
    narrative: 'O sucesso da integração abre caminho para conectar mais sistemas, mais hospitais e mais fluxos de dados — fortalecendo o ecossistema de saúde digital do Brasil.',
    before: 'Integração pontual, difícil de expandir para novos sistemas',
    after: 'Arquitetura extensível, pronta para integrar novos parceiros e fluxos',
    color: '#059669',
  },
];
...
export const SirioLibanesDemo: React.FC = () => {
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          ...
          {isLast && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200">
              <p className="text-sm text-emerald-700 font-medium">Dados que conversam. Sistemas que colaboram. Pacientes que se beneficiam.</p>
              <p className="text-xs text-slate-500 mt-1">dataRain × Sírio-Libanês — interoperabilidade que salva vidas.</p>
            </motion.div>
          )}
        </div>
      </div>
