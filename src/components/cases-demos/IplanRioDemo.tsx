...
import { Building2, Ticket, FileText, Users, ChevronLeft, ChevronRight } from 'lucide-react';
...
const steps: JourneyStep[] = [
  { id: 'client', icon: Building2, title: 'Prefeitura do Rio de Janeiro', narrative: 'IplanRio é responsável pela TI da Prefeitura do RJ. A dataRain presta suporte contínuo de infraestrutura AWS.', before: 'Infraestrutura AWS sem suporte gerenciado', after: 'Suporte contínuo com SLA e relatórios mensais', color: '#0d47a1' },
  { id: 'tickets', icon: Ticket, title: 'Gestão de tickets', narrative: 'Sistema de chamados técnicos com priorização e SLA. Resolução de incidentes e requisições de infraestrutura.', before: 'Chamados sem priorização e sem SLA', after: 'Gestão de tickets com SLA definido', color: '#7c3aed' },
  { id: 'reports', icon: FileText, title: 'Relatórios mensais', narrative: 'Reports mensais com métricas de atendimento, análise de tickets e recomendações de melhorias.', before: 'Sem visibilidade sobre operações', after: 'Relatórios mensais com métricas e recomendações', color: '#0891b2' },
  { id: 'subprojects', icon: Users, title: 'Subprojetos operacionais', narrative: 'Suporte ao IPP (migração e VPN) e Instituto João Goulart (Moodle na AWS). Infraestrutura compartilhada.', before: 'Subprojetos sem suporte dedicado', after: 'IPP e João Goulart operacionais na AWS', color: '#059669' },
];

export const IplanRioDemo: React.FC = () => {
    ...
    <div className="w-full max-w-4xl mx-auto my-8">
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">Suporte contínuo AWS ativo para Prefeitura do Rio de Janeiro.</p></motion.div>}
        </div>
      </div>
    </div>
  );
