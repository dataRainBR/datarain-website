...
import { Scale, Container, Search, TrendingDown, ChevronLeft, ChevronRight } from 'lucide-react';
...
const steps: JourneyStep[] = [
  { id: 'client', icon: Scale, title: 'Ministério Público do RN', narrative: 'O MPRN buscava modernizar infraestrutura com Kubernetes gerenciado e explorar rastreio de dados pessoais para LGPD.', before: 'Infraestrutura tradicional sem containers', after: 'EKS Fargate com perfis dev/prd na AWS', color: '#1a237e' },
  { id: 'eks', icon: Container, title: 'EKS Fargate serverless', narrative: 'Amazon EKS com AWS Fargate para execução serverless de containers. Perfis separados dev e prd com namespaces isolados.', before: 'Sem ambiente Kubernetes gerenciado', after: 'EKS Fargate com perfis dev/prd isolados', highlight: 'Serverless — sem EC2 para gerenciar', color: '#7c3aed' },
  { id: 'poc', icon: Search, title: 'POC rastreio de dados pessoais', narrative: 'Prova de conceito para sistema de rastreio de dados pessoais alinhado com requisitos da LGPD.', before: 'Sem visibilidade sobre dados pessoais armazenados', after: 'POC de rastreio de dados pessoais para LGPD', color: '#059669' },
  { id: 'finops', icon: TrendingDown, title: 'Otimização de custos', narrative: 'Análise de billing e recomendações de otimização. Fargate elimina necessidade de gerenciar instâncias EC2.', before: 'Custos não otimizados sem análise de billing', after: 'Fargate serverless + análise de billing otimizada', color: '#d97706' },
];

export const MPRNDemo: React.FC = () => {
    ...
    <div className="w-full max-w-4xl mx-auto my-8">
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">EKS Fargate operacional com perfis dev/prd e POC de rastreio executada.</p></motion.div>}
        </div>
      </div>
    </div>
  );
