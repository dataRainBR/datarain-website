...
import { Heart, Container, Globe, FileText, ChevronLeft, ChevronRight } from 'lucide-react';
...
const steps: JourneyStep[] = [
  { id: 'client', icon: Heart, title: 'Plataforma de saúde digital', narrative: 'BippCare (bipp.care) conecta pacientes a profissionais de saúde com aplicação web containerizada.', before: 'Aplicação sem infraestrutura cloud', after: 'App containerizado na AWS com EC2', color: '#00897b' },
  { id: 'docker', icon: Container, title: 'Docker Compose multi-serviço', narrative: 'Frontend Next.js/React + Backend NestJS/TypeScript + PostgreSQL + Redis, todos em containers Docker Compose.', before: 'Serviços sem containerização', after: 'Docker Compose: Next.js + NestJS + PostgreSQL + Redis', color: '#7c3aed' },
  { id: 'nginx', icon: Globe, title: 'NGINX + SSL/TLS', narrative: 'NGINX como reverse proxy com SSL/TLS para tráfego seguro. VPC com Security Groups restritivos.', before: 'Sem reverse proxy e sem SSL', after: 'NGINX com SSL/TLS + VPC segura', color: '#dc2626' },
  { id: 'docs', icon: FileText, title: 'Documentação técnica', narrative: 'Documentação completa da infraestrutura com procedimentos de deploy e manutenção.', before: 'Sem documentação de infraestrutura', after: 'Documentação completa de deploy e manutenção', color: '#059669' },
];

export const BippCareDemo: React.FC = () => {
    ...
    <div className="w-full max-w-4xl mx-auto my-8">
      <div className="relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-lg">
        ...
        <div className="relative p-5 md:p-8 space-y-6">
          ...
          {isLast && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-4 rounded-xl bg-emerald-50 border border-emerald-200"><p className="text-sm text-emerald-700 font-medium">App de saúde containerizado operacional na AWS com SSL e documentação.</p></motion.div>}
        </div>
      </div>
    </div>
  );
